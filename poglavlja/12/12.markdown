...
U sekciji 12.3.1, diskutovali smo o metodu `from()` iz interfejsa `AbstractQuery` kao i o ulogama korenova upita u formiranju definicije upita. Sada \'cemo pro\v siriti tu diskusiju i prikazati na\v cine na koje se operacije spajanja izra\v zavaju u JPA Criteria API.
...
to be an inner join. Just as in HQL, multiple joins may be associated with the same
...
Joins across collection relationships that use Map are a special case. HQL uses the
...
functions supported by the HQL language as well as other features specific to the
Criteria API. Table 9-2, Table 9-3, Table 9-4, and Table 9-5 summarize the mapping
between HQL operators, expressions, and functions to their equivalent methods on
...
In addition to the straight translation of HQL operators, expressions, and functions,
...
explore those parts of the Criteria API that have no equivalent in HQL.
...
Parameter handling for Criteria API queries is different from HQL. Whereas HQL strings
...
The equivalent HQL query with only Project criteria would be:
...
joining directly to the list of projects specific to that employee. In HQL, we would write
this as follows:
...
returned from the in() method. Consider the following HQL query:
...
complex example, in the previous chapter, we demonstrated a HQL query using an IN
...
from HQL. Unfortunately, “else” is already a keyword in Java, so “otherwise” must be
...
The last example we cover in this section concerns the HQL COALESCE expression.
...
Not to be confused with the built-in functions of HQL, criteria function expressions are
the Criteria API equivalent of the FUNCTION keyword in HQL. They allow native SQL
...
the orderBy() method determines the generation of SQL. The equivalent HQL for the
...
API equivalent of the GROUP BY and HAVING clauses from HQL, respectively. Both
...
{% include zadatak.html broj="12.1" tekst="Napisati Java aplikaciju koja kori\v s\'cenjem biblioteke Hibernate ." %}

{% include zadatak.html broj="12.2" tekst="Napisati Java aplikaciju koja kori\v s\'cenjem biblioteke Hibernate ." %}