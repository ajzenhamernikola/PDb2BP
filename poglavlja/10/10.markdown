...
{% include zadatak.html broj="10.1" tekst="Napisati Java aplikaciju koja kori\v s\'cenjem biblioteke Hibernate unosi podatak o novom smeru u tabeli `SMER` sa podacima iz naredne tabele." %}
...
{% include zadatak.html broj="10.2" tekst="Napisati Java aplikaciju koja kori\v s\'cenjem biblioteke Hibernate bri\v se podatke o smeru sa identifikatorom 300 iz tabele `SMER`." %}

Re\v senje: Kako smo ve\'c napisali OR preslikavanje izme\dj u klase `Smer` i tabele `SMER`, sve \v sto je potrebno da uradimo jeste da kreiramo stati\v cki metod `deleteSmer()` u glavnoj klasi `Main.java` i pozovemo ga u metodi `main`. Prikazujemo dopunjenu klasu `Main.java`:
...
{% include zadatak.html broj="10.3" tekst="Napisati Java aplikaciju koja kori\v s\'cenjem biblioteke Hibernate implementira unos podataka o novom ispitnom roku (jun 2019. godine) u tabelu `ISPITNI_ROK`." %}
...
include_source(primeri/poglavlje_10/src/zadatak_10_3/IspitniRokId.java, java)

Zatim je potrebno specifikovati instancu ove klase kao primarni ključ klase `IspitniRok.java`, 
koju takođe kreiramo:

include_source(primeri/poglavlje_10/src/zadatak_10_3/IspitniRok.java, java)

Definisane klase se sada jednostavno koriste u metodu `insertIspitniRok()` proširene klase
`Main.java` iz prethodnog primera:

include_source(primeri/poglavlje_10/src/zadatak_10_3/Main.java, java)

Ne zaboravimo da dodamo `IspitniRok` kao anotiranu klasu u konfiguraciju:

include_source(primeri/poglavlje_10/src/zadatak_10_3/HibernateUtil.java, java)

{% include zadatak.html broj="10.4" tekst="Napisati Java aplikaciju koja kori\v s\'cenjem biblioteke Hibernate bri\v se ispitni rok jun 2019. iz tabele `ISPITNI_ROK`." %}

Re\v senje: Sa svim znanjem do sada, ovaj zahtev se trivijalno implementira. Prikazujemo samo
ažuriranu klasu `Main.java` i skrećemo pažnju na njen metod `deleteIspitniRok()` koji
ilustruje rešenje ovog zahteva.

include_source(primeri/poglavlje_10/src/zadatak_10_4/HibernateUtil.java, java)
