...
```c
EXEC SQL SELECT SUM(ZARADA) INTO :ukupnaZarada FROM ZAPOSLENI;
```
...
EXEC SQL 
    SELECT 
        MIN(COALESCE(DATUM_PISMENOG, DATUM_USMENOG)), 
...
```sql
SELECT 
    nullKolona INTO :maticnaPromenljiva :indikatorskaPromenljiva
...
```
...
Nakon izvršavanja svake SQL naredbe, sistem daje povratni kod kroz C makroe `SQLCODE` i `SQLSTATE`. `SQLCODE` predstavlja marko koji se razvija celi broj koja na neki način sumira izvršavanje SQL naredbe, dok je `SQLSTATE` makro koji se razvija u nisku od 5 karaktera, koja detaljnije opisuje česte kodove za greške među različitim softverskim rešenjima vezane za relacione baze od strane IBM-a. Mi ćemo se najčešće oslanjati na makro `SQLCODE` prilikom obrade grešaka.
...
> Zadatak 2.1. Napisati C/SQL program koji ispisuje maksimalni indeks iz tabele ISPIT.
...
```shell
./prevodjenje zadatak-2-1 vstud student abcdef
```

Tokom ovog procesa možemo videti da li je došlo do nekih grešaka u fazi prevođenja. Ukoliko je sve prošlo kako treba, kreirana je izvršna datoteka `zadatak-2-1` koja se pokreće kao i svaki drugi izvršni C program:

```shell
./zadatak-2-1
```

> Zadatak 2.2. Napisati C/SQL program koji ispisuje indeks, ime, prezime, ime oca (ukoliko je navedeno u bazi) i datum rođenja (ukoliko je navedeno u bazi) za studenta sa maksimalnim indeksom iz tabele `ISPIT`.
...
int main()
{
    ...
    EXEC SQL 
        ...
        FROM 
            DOSIJE
        WHERE 
            INDEKS = (SELECT MAX(INDEKS) FROM DOSIJE);
    is_error("Izdvajanje podatka o studentu sa najvecim indeksom");
...
> Zadatak 2.3. Napisati C/SQL program koji ispisuje indeks, ime, prezime, ime oca (ukoliko je navedeno u bazi) i datum rođenja (ukoliko je navedeno u bazi) za studenta čiji se broj indeksa učitava sa standardnog ulaza.
...
## 2.4 Naredbe `INSERT`, `UPDATE` i `DELETE`

Do sada smo diskutovali o programima u kojima smo dohvatali tačno jedan red u tabeli (ili, eventualno, nijedan), korišćenjem naredbe `SELECT INTO`. U nastavku slede zadaci u kojem ćemo demonstrirati upotrebu naredbi `INSERT`, `UPDATE` i `DELETE`. Ove naredbe se jednostavno implementiraju njihovim navođenjem nakon `EXEC SQL`.

> Zadatak 2.4. Napisati C/SQL program u kojem su implementirane naredne funkcije. Napisati i funkciju `int main()` koja testira napisane funkcije.
...
void unesi_novi_predmet()
{
    ...
    EXEC SQL SELECT id_predmeta, sifra, naziv, broj_semestara, bodovi
        INTO :id_predmeta, :sifra, :naziv, :broj_semestara, :bodovi
        FROM PREDMET
...
> Zadatak 2.5. Napisati C/SQL program koji ispisuje podatke za predmet čiji je identifikator `640`.

> Zadatak 2.6. Napisati C/SQL program koji ispisuje broj studenata koji su upisali studije u godini koja se unosi sa standardnog ulaza.
...
> Zadatak 2.8. Napisati C/SQL program kojim se dodaje da je za polaganje predmeta čiji je identifikator `720` uslov da se položi predmet čiji je identifikator `655`.

> Zadatak 2.9. Napisati C/SQL program kojim se u tabelu `NIVO_KVALIFIKACIJE` dodaje novi nivo čiji se identifikator, naziv i stepen unose sa standardnog ulaza.

> Zadatak 2.10. Napisati C/SQL program kojim se svim obaveznim predmetima na smeru "Informatika" povećava broj semestra za 1.
...
> Zadatak 2.12. Napisati C/SQL program kojim se u tabeli `USLOVNI_PREDMET` brišu svi redovi koji se odnose na predmete koji su uslovni da bi se položio predmet čiji se identifikator unosi sa standardnog ulaza.

> Zadatak 2.13. Napisati C/SQL program kojim se brišu svi podaci o ispitima za studenta čiji se indeks unosi sa standardnog ulaza.
