...
Pre nego što započnemo detaljnu diskusiju o transakcijama, obratićemo pažnju na dve vrlo važne operacije kada je u pitanju rad sa transakcijama. Ono što do sada nismo napomenuli jeste da, čak i kad naš C/SQL program uspešno izvrši neku naredbu, ona neće biti u tom trenutku trajno upisana u bazu podataka. To znači da neki drugi C/SQL program, ukoliko želi da pročita "izmenjenu" vrednost, ne'ce biti u stanju to da uradi pre nego što prvi program ne potvrdi svoje izmene. Naredna slika ilustruje ovu situaciju: u nekom trenutku Program 1 vrši izmenu Reda 1 u Tabeli, neposredno zatim Program 2 želi da izvrši čitanje te "izmenjene" vrednosti. Međutim, pošto u tom trenutku Program 1 nije potvrdio te izmene, Program 2 ne vidi izmene i čita staru vrednost. Tek nakon čitanja Programa 2, Program 1 vrši potvrđivanje izmena i red biva izmenjen.

![Čitanje izmene reda koja nije potvrđena može dovesti do nekorektnog ponašanja](./Slike/CitanjePrePotvrdjivanja.png)

SQL jezik definiše naredne dve naredbe za rad sa transakcijama:

- Naredba `COMMIT` služi za potvrđivanje izmena koje su do tada bile izvršene nad bazom podataka. Sve načinjene izmene se trajno pamte u bazi podataka i svi ostali procesi dobijaju mogućnost da vide načinjene izmene.

- Naredba `ROLLBACK` služi za poništavanje izmena koje su do tada bile izvršene nad bazom podataka. Izvršavanjem ove naredbe možemo poništiti sve one akcije koje do trenutka poništavanja nisu prethodno bile potvrđene. 

Pogledajmo efekat ovih naredbi na narednom zadatku.
